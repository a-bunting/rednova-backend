...
router.get('/getPlayableGalaxyList', (req, res, next) => {
    ...
    db.query(getSql, (e, data) => {
        ...
        if(data.length > 0) {
            ...
            db.query(userGalaxiesQuery, (e, userQueryResult) => {
...
router.post('/joinGalaxy', checkAuth, (req, res, next) => {
    ...
    db.query(galaxyQuery, (e, result) => {
        ...
        if(result[0].length === 1 && result[1][0].cnt === 0 && !e) {
            ...
            db.query(insertQuery, (e, insertResult) => {
                if(!e) {
                ...
                } else res.status(400).json({ error: true, message: `Error: ${e}`, data: {} })
            })

        } else res.status(400).json({ error: true, message: `The galaxy could not be found or cannot be played`, data: {} })
...
router.get('/getUserGalaxyData', checkAuth, (req, res, next) => {
    ...
    db.query(checkUserExistsInGalaxyQuery, (e, result) => {
        ...
        if(result.length === 1) {
            ...
            db.query(sectorQuery, (e, galaxyResult) => {
                if(e) console.log(`Error: ${e}`)
...
router.post('/moveTo', checkAuth, (req, res, next) => {
    ...
    db.query(sectorQuery, (e, queryResult) => {
        if(e) console.log(`Error: ${e}`);
