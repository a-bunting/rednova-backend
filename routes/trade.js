            ...
            connection.query(sql, (e, result) => {
                if(!e) {
                    ...
                    for(let i = 0 ; i < traderoute.stages.length ; i++) {
                        ...
                        connection.query(stagesSql, [insert], (e, stageResult) => {
                            connection.destroy();
                            
                            console.log(stagesSql);
                        if(!e) {
                    ...
                    connection.destroy();
                    res.status(400).json({ error: true, message: `Error Querying Database: SQL ${sql}`, data: {}});
                }
            })
        } else {
            connection.destroy();
            res.status(400).json({ error: true, message: 'Connection Error to Database', data: {}});
        }
        ...
        if(!conErr) {
            connection.query(sql, (e, result) => {
                ...
                if(!e && result.affectedRows === 1) {
                    ...
                    connection.query(stageDelete, (e, r) => {
                        connection.destroy();
                    ...
                    connection.destroy();
                    res.status(200).json({ error: true, message: 'Error Deleting Row', data: {}})
                };
            })
        } else { 
            connection.destroy();
            res.status(200).json({ error: true, message: 'Connection Error to Database', data: {}});
        }
...
router.post('/executeTradeRoutes', checkAuth, (req, res, next) => {
    ...
    const tradeSql = `
        SELECT trade__stages.stage, trade__stages.action, trade__stages.detail 
        FROM trade__route 
    ...
    connection.connect(conErr => {
        if(!conErr) {
            connection.query(tradeSql, (e, result) => {
                if(!e) {
                    ...
                    connection.query(sql, (e, planetaryData) => {
                        if(!e) {
                            ...
                            for(let i = 0 ; i < planetData.length ; i++) {
                                for(let o = 0 ; o < planetData[i].goods.length ; o++) {
                                    ...
                                    // good.push([+data.id]);
                                    // galaxy.push([+galaxyId]);
                                    // sector.push([+planetData[i].sectorid]);
                                    // planet.push([+planetData[i].index]);
                                }
                            }
                            ...
                            console.log(`${updateShip} ; ${updatePlanetData}`);

                            connection.query(`${updateShip} ; ${updatePlanetData}`, [values], (e, r) => {
                                connection.destroy(); // temporary until the update sql is written...

                                console.log(e, r);

                                // also needs to ensure the player has a display and data for the new sector they might be in...
                                if(!e) {
                                    console.log(returnLog, +turnsUsed, +iterationsRun);
                                    res.status(200).json({ error: false, message: '', data: { log: returnLog, turns: +turnsUsed, iterations: +iterationsRun }});
                                } else {
                            ...
                            connection.destroy();
                            res.status(400).json({ error: true, message: 'Error Querying Database (A2)', data: {}}); 
                        }
                    })

                } else {
                    connection.destroy();
                    res.status(400).json({ error: true, message: 'Error Querying Database (A1)', data: {}});
                }
            })
        } else {
            connection.destroy();
